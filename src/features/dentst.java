...
public class dentst extends javax.swing.JFrame {
 ...
 private static final String SECTION_TREATMENT = "treatment";
 private static final String SECTION_SETTINGS = "settings";
 private static final String SECTION_PROFILE = "profile";
 private String currentSection = SECTION_DASHBOARD;
 private int selectedPatientAppointmentId = -1;
...
public dentst() {
    ...
    loadDashboard();
    loadPatientsOfDentist();
    Table_PatientsofDentist.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
    Table_PatientsofDentist.setRowSelectionAllowed(true);
    setAppointmentButtonsEnabled(false);

    Table_PatientsofDentist.getSelectionModel().addListSelectionListener(evt -> {
        if (!evt.getValueIsAdjusting()) {
            int row = Table_PatientsofDentist.getSelectedRow();
            if (row >= 0) {
                selectedPatientId = Integer.parseInt(Table_PatientsofDentist.getValueAt(row, 0).toString());
                selectedPatientAppointmentId = getSelectedPatientAppointmentId(row);
                setAppointmentButtonsEnabled(selectedPatientAppointmentId > 0);
            } else {
                selectedPatientId = -1;
                selectedPatientAppointmentId = -1;
                setAppointmentButtonsEnabled(false);
            }
        }
    });
...
set_specialization.addItem("General Dentistry");
set_specialization.addItem("Orthodontics");
set_specialization.addItem("Pediatric Dentistry");
set_specialization.addItem("Endodontics");
set_specialization.addItem("Periodontics");

    // ✅ Ensure all schedule checkboxes are unchecked at startup
...
sunCheck.setSelected(false);
     ...
     loadSchedule();
     loadScheduleTable();  // ✅ populate JTable with DB values
    ...
    // 👉 Insert this here
    saveSchedule.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR)); // makes it look clickable
    saveSchedule.addMouseListener(new java.awt.event.MouseAdapter() {
        @Override
        public void mouseClicked(java.awt.event.MouseEvent evt) {
            saveScheduleMouseClicked(evt); // call your method
        }
    });
}
...
private void sendPatientsBackgroundToBack() {
    if (patient != null && jLabel57 != null) {
        patient.setComponentZOrder(jLabel57, patient.getComponentCount() - 1);
        patient.repaint();
    }
}

private void registerSectionTracking() {
    dashTb.addChangeListener(evt -> {
        int selectedIndex = dashTb.getSelectedIndex();
        switch (selectedIndex) {
            case 0:
                setCurrentSection(SECTION_DASHBOARD);
                break;
            case 1:
                setCurrentSection(SECTION_SCHEDULE);
                break;
            case 2:
                setCurrentSection(SECTION_PATIENTS);
                break;
            case 3:
                setCurrentSection(SECTION_TREATMENT);
                break;
            case 4:
            case 5:
                setCurrentSection(SECTION_PROFILE);
                break;
            case 7:
                setCurrentSection(SECTION_SETTINGS);
                break;
            default:
                break;
        }
...
private int resolveDentistId() {
    int dentistId = session.getDentistId();
    if (dentistId > 0) {
        return dentistId;
    }
    ...
    try (Connection conn = config.connectDB();
         PreparedStatement ps = conn.prepareStatement(
             "SELECT dentist_id FROM tbl_dentists WHERE acc_id = ?"
         )) {
        ps.setInt(1, session.getId());
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
            dentistId = rs.getInt("dentist_id");
            session.setDentistId(dentistId);
            return dentistId;
        }
    } catch (Exception e) {
        e.printStackTrace();
...
private void loadDashboard() {
    int dentistId = resolveDentistId();
    if (dentistId <= 0) {
        tp.setText("0");
        jLabel14.setText("0");
        inProg.setText("0");
        jLabel16.setText("0");
        upcoming.setText("No schedule available");
        return;
    }
    ...
    try (Connection conn = config.connectDB()) {
        ...
        try (PreparedStatement pst = conn.prepareStatement(
                "SELECT COUNT(*) AS total FROM tbl_appointments a " +
                "WHERE (a.dentist_id = ? OR a.dentist_id = ?) " +
        ...
        try (PreparedStatement pst = conn.prepareStatement(
                "SELECT COUNT(*) AS total FROM tbl_appointments a " +
                "WHERE (a.dentist_id = ? OR a.dentist_id = ?) " +
                "AND a.app_status = 'In Progress'"
        )) {
            pst.setInt(1, dentistId);
            pst.setInt(2, session.getId());
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
                    inProg.setText(rs.getString("total"));
                }
            }
        }

        // Active patients count (distinct patient records)
        try (PreparedStatement pst = conn.prepareStatement(
                "SELECT COUNT(DISTINCT COALESCE(p.pat_id, a.pat_id)) AS total " +
                "FROM tbl_appointments a " +
                "LEFT JOIN tbl_patients p ON p.pat_id = a.pat_id " +
                "WHERE (a.dentist_id = ? OR a.dentist_id = ?) " +
                "AND a.app_status <> 'Cancelled'"
        )) {
            pst.setInt(1, dentistId);
        ...
        loadTodaySchedule();
        loadDashboardRecentActivity(conn, dentistId);
    } catch (Exception e) {
        e.printStackTrace();
...
private void loadDashboardRecentActivity(Connection conn, int dentistId) {
    javax.swing.table.DefaultTableModel model = new javax.swing.table.DefaultTableModel(
        new Object[]{"Patient", "Service", "Date", "Time", "Status"}, 0
    ) {
        @Override
    ...
    try (PreparedStatement pst = conn.prepareStatement(
            "SELECT COALESCE(NULLIF(TRIM(p.pat_name), ''), 'Unknown') AS patient_name, " +
            "a.app_service, a.app_date, a.app_time, a.app_status " +
            "FROM tbl_appointments a " +
            "LEFT JOIN tbl_patients p ON p.pat_id = a.pat_id " +
            "WHERE (a.dentist_id = ? OR a.dentist_id = ?) " +
...
private int getSelectedPatientAppointmentId(int row) {
    if (row < 0) {
        return -1;
    }

    javax.swing.table.TableModel model = Table_PatientsofDentist.getModel();
    int appIdColumn = model.getColumnCount() - 1;
    if (appIdColumn < 0) {
        return -1;
    }

    Object rawValue = model.getValueAt(row, appIdColumn);
    if (rawValue == null) {
        return -1;
    }
...
private void updateAppointmentStatus(int appointmentId, String status) {
    if (appointmentId <= 0) {
        JOptionPane.showMessageDialog(this, "Please select a valid appointment first.");
        return;
    }

    try (Connection conn = config.connectDB()) {
        PreparedStatement pst = conn.prepareStatement(
            "UPDATE tbl_appointments SET app_status=? WHERE app_id=?"
        );
        pst.setString(1, status);
        pst.setInt(2, appointmentId);
        pst.executeUpdate();

        logAction(conn, session.getId(), "dentist", "Update Appointment",
                  "Changed appointment " + appointmentId + " status to " + status);
...
private void setAppointmentStartTime(int appointmentId) {
    if (appointmentId <= 0) {
        JOptionPane.showMessageDialog(this, "Please select a valid appointment first.");
        return;
    }
    ...
    try (Connection conn = config.connectDB()) {
        PreparedStatement pst = conn.prepareStatement(
            "UPDATE tbl_appointments SET app_time=? WHERE app_id=?"
        );
        pst.setString(1, newTime.trim());
        pst.setInt(2, appointmentId);
        pst.executeUpdate();

        logAction(conn, session.getId(), "dentist", "Update Appointment",
                  "Set appointment " + appointmentId + " time to " + newTime.trim());
        JOptionPane.showMessageDialog(this, "Appointment start time updated.");
        loadPatientsOfDentist();
        loadTodaySchedule();
    } catch (Exception ex) {
        JOptionPane.showMessageDialog(this, "Error updating time: " + ex.getMessage());
    }
}

private void handleSelectedAppointmentAction(String action) {
    int row = Table_PatientsofDentist.getSelectedRow();
    if (row == -1) {
    ...
    int appointmentId = getSelectedPatientAppointmentId(row);
    if (appointmentId <= 0) {
        JOptionPane.showMessageDialog(this, "Unable to identify the selected appointment.");
        return;
    }

    if ("Set Start Time".equals(action)) {
        setAppointmentStartTime(appointmentId);
    } else {
        updateAppointmentStatus(appointmentId, action);
    }
}

private void setAppointmentButtonsEnabled(boolean enabled) {
    btnConfirm.setEnabled(enabled);
    btnReject.setEnabled(enabled);
    btnComplete.setEnabled(enabled);
    btnSetStartTime.setEnabled(enabled);
}
...
private ImageIcon createCircularImageIcon(ImageIcon icon, int width, int height) {
...
private javax.swing.table.DefaultTableModel createPatientsTableModel() {
    return new javax.swing.table.DefaultTableModel(
        new Object[]{"Patient ID", "Name", "Service", "Date", "Time", "Status", "Appointment ID"}, 0
    ) {
        @Override
        public boolean isCellEditable(int row, int column) {
            return false;
...
private void loadPatientsOfDentist() {
    javax.swing.table.DefaultTableModel model = createPatientsTableModel();
    Table_PatientsofDentist.setModel(model);

    try (Connection conn = config.connectDB()) {
        String sql = "SELECT a.app_id AS app_id, " +
                     "COALESCE(p.pat_id, a.pat_id) AS patient_id, " +
                     "COALESCE(NULLIF(TRIM(p.pat_name), ''), 'Unknown Patient') AS patient_name, " +
                     "a.app_service AS app_service, " +
                     "a.app_date AS app_date, " +
                     "a.app_time AS app_time, " +
                     "a.app_status AS app_status " +
                     "FROM tbl_dentists d " +
                     "JOIN tbl_appointments a ON (a.dentist_id = d.dentist_id OR a.dentist_id = d.acc_id) " +
                     "LEFT JOIN tbl_patients p ON p.pat_id = a.pat_id " +
                     "WHERE d.acc_id = ? " +
                     "ORDER BY a.app_date DESC, a.app_time DESC";
        ...
        Table_PatientsofDentist.setModel(model);
        Table_PatientsofDentist.setRowHeight(28);
        Table_PatientsofDentist.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        if (Table_PatientsofDentist.getColumnModel().getColumnCount() > 6) {
            Table_PatientsofDentist.getColumnModel().getColumn(6).setMinWidth(0);
            Table_PatientsofDentist.getColumnModel().getColumn(6).setMaxWidth(0);
            Table_PatientsofDentist.getColumnModel().getColumn(6).setPreferredWidth(0);
        }
...
private void loadTreatmentTable() {
    javax.swing.table.DefaultTableModel model = new javax.swing.table.DefaultTableModel(
        new Object[]{"Appointment ID", "Patient ID", "Service", "Date", "Time", "Status"}, 0
    ) {
        @Override
        public boolean isCellEditable(int row, int column) {
            return false;
    ...
    MYappointment.setModel(model);

    try (Connection conn = config.connectDB()) {
        String sql = "SELECT a.app_id, a.pat_id, a.app_service, a.app_date, a.app_time, a.app_status " +
                     "FROM tbl_dentists d " +
                     "JOIN tbl_appointments a ON (a.dentist_id = d.dentist_id OR a.dentist_id = d.acc_id) " +
                     "WHERE d.acc_id = ? " +
                     "ORDER BY a.app_date DESC, a.app_time DESC";
        ...
        MYappointment.setRowHeight(28);
        MYappointment.setFont(new Font("Segoe UI", Font.PLAIN, 13));
    } catch (Exception e) {
    ...
    private void initComponents() {
        ...
        Table_PatientsofDentist = new javax.swing.JTable();
        ...
        dashbox.add(sett, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 150, 170, 30));
        ...
        dashbox.add(lg, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 190, 170, 30));
        ...
        jLabel25.setText("_________________________________________________________");
        sched.add(jLabel25, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 70, 430, -1));
        ...
        sched.add(jLabel46, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 110, -1, -1));
        ...
        startTimeCombo.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(153, 153, 153), 2));
        sched.add(startTimeCombo, new org.netbeans.lib.awtextra.AbsoluteConstraints(320, 330, 110, 30));

        jLabel54.setFont(new java.awt.Font("Times New Roman", 1, 18)); // NOI18N
        jLabel54.setText("Work Hours");
        sched.add(jLabel54, new org.netbeans.lib.awtextra.AbsoluteConstraints(200, 120, -1, 40));
        ...
        sched.add(jScrollPane4, new org.netbeans.lib.awtextra.AbsoluteConstraints(210, 180, 410, 60));
        ...
        endTimeCombo.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(153, 153, 153), 2));
        sched.add(endTimeCombo, new org.netbeans.lib.awtextra.AbsoluteConstraints(320, 380, 110, 30));

        jLabel26.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        jLabel26.setText("Start Time:");
        sched.add(jLabel26, new org.netbeans.lib.awtextra.AbsoluteConstraints(230, 330, 70, 30));
        ...
        jPanel9.add(monCheck, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 100, 30));
        ...
        jPanel9.add(tueCheck, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 40, 100, 30));
        ...
        jPanel9.add(wedCheck, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 80, 100, 30));
        ...
        jPanel9.add(thurCheck, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 120, 100, 30));
        ...
        jPanel9.add(friCheck, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 160, 100, 30));
        ...
        jPanel9.add(satCheck, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 200, 100, 30));
        ...
        jPanel9.add(sunCheck, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 240, 100, 30));

        jLabel39.setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/nn.jpg"))); // NOI18N
        jLabel39.setVerticalAlignment(javax.swing.SwingConstants.TOP);
        jPanel9.add(jLabel39, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, -10, 100, 290));

        sched.add(jPanel9, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 160, 100, 270));

        jLabel38.setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/12 (2).jpg"))); // NOI18N
        jLabel38.setToolTipText("");
        sched.add(jLabel38, new org.netbeans.lib.awtextra.AbsoluteConstraints(430, 40, 180, 110));

        jLabel53.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        jLabel53.setText("End Time:");
        sched.add(jLabel53, new org.netbeans.lib.awtextra.AbsoluteConstraints(230, 390, -1, -1));

        jPanel10.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        jLabel55.setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/a.jpg"))); // NOI18N
        jPanel10.add(jLabel55, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 430, 100));

        sched.add(jPanel10, new org.netbeans.lib.awtextra.AbsoluteConstraints(200, 160, 430, -1));
        ...
        Table_PatientsofDentist.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {},
                {},
                {},
        ...
        jScrollPane2.setViewportView(Table_PatientsofDentist);

        patient.add(jScrollPane2, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 130, 620, 350));

        btnConfirm = new javax.swing.JButton();
        btnReject = new javax.swing.JButton();
        btnComplete = new javax.swing.JButton();
        btnSetStartTime = new javax.swing.JButton();
        ...
        btnComplete.setBackground(new java.awt.Color(0, 102, 204));
        btnComplete.setFont(new java.awt.Font("Times New Roman", 1, 14)); // NOI18N
        ...
        btnSetStartTime.setBackground(new java.awt.Color(255, 153, 51));
        btnSetStartTime.setFont(new java.awt.Font("Times New Roman", 1, 14)); // NOI18N
        btnSetStartTime.setForeground(new java.awt.Color(255, 255, 255));
        btnSetStartTime.setText("Set Start Time");
        btnSetStartTime.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnSetStartTimeActionPerformed(evt);
            }
        });
        ...
        patient.add(jLabel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 30, -1, 50));

        jLabel7.setFont(new java.awt.Font("Times New Roman", 0, 14)); // NOI18N
        jLabel7.setForeground(new java.awt.Color(204, 204, 204));
        jLabel7.setText("_______________________________________________________________________________________");
        patient.add(jLabel7, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 60, -1, -1));

        jLabel57.setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/nn.jpg"))); // NOI18N
        patient.add(jLabel57, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 640, 500));

        patient.add(btnConfirm, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 90, 140, 30));
        patient.add(btnReject, new org.netbeans.lib.awtextra.AbsoluteConstraints(160, 90, 140, 30));
        patient.add(btnComplete, new org.netbeans.lib.awtextra.AbsoluteConstraints(310, 90, 140, 30));
        patient.add(btnSetStartTime, new org.netbeans.lib.awtextra.AbsoluteConstraints(460, 90, 170, 30));
        ...
        set_specialization.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                set_specializationMouseClicked(evt);
            }
        });
        ...
        jLabel36.setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/bor (3).jpg"))); // NOI18N
        mp.add(jLabel36, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 200, 150, 40));
    ...
    private void btnConfirmActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnConfirmActionPerformed
        handleSelectedAppointmentAction("Confirmed");
    }//GEN-LAST:event_btnConfirmActionPerformed

    private void btnRejectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRejectActionPerformed
        handleSelectedAppointmentAction("Rejected");
    }//GEN-LAST:event_btnRejectActionPerformed

    private void btnCompleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCompleteActionPerformed
        handleSelectedAppointmentAction("Completed");
    }//GEN-LAST:event_btnCompleteActionPerformed

    private void btnSetStartTimeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSetStartTimeActionPerformed
        handleSelectedAppointmentAction("Set Start Time");
    }//GEN-LAST:event_btnSetStartTimeActionPerformed
 ...
 dashTb.setSelectedIndex(4);      
    ...
    try {
        ...
        try (Connection con = config.connectDB();
            ...
            pst.setInt(4, session.getDentistId()); // ✅ correct field
    ...
    private void set_specializationMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_set_specializationMouseClicked
        // TODO add your handling code here:
    }//GEN-LAST:event_set_specializationMouseClicked
    ...
    try (Connection con = config.connectDB();
        ...
        if (updated > 0) {
            ...
            try (PreparedStatement logStmt = con.prepareStatement(
                ...
                logStmt.setString(2, "dentist");
                logStmt.setString(3, "Updated schedule");
                logStmt.setString(4, "Updated schedule for dentist: " + workDays + " (" + start + " - " + end + 
...
private void loadSchedule() {
    try (Connection con = config.connectDB();
        ...
        pst.setInt(1, session.getDentistId()); // ✅ use dentistId here
        ...
        if (rs.next()) {
            ...
            LocalTime startTime = tryParseTime(start);
            LocalTime endTime   = tryParseTime(end);
            ...
            // Defaults if no schedule exists
            startTimeCombo.setSelectedItem("8:00 AM");
            endTimeCombo.setSelectedItem("5:00 PM");
            monCheck.setSelected(false);
            tueCheck.setSelected(false);
...
/**
 * Utility to parse time strings from DB with mixed formats
 */
private LocalTime tryParseTime(String timeStr) {
...
private void loadProfileEdit() {
    try (Connection con = config.connectDB()) {
        ...
        if (rsAcc.next()) {
            ...
            // --- Photo handling (BLOB or TEXT path) ---
            ImageIcon icon = loadImageIconFromBytes(rsAcc.getBytes("acc_pic"));
            if (icon == null) {
                icon = loadImageIconFromPath(rsAcc.getString("acc_pic"));
            }
            if (icon == null) {
...
private void loadProfileDisplay() {
    try (Connection conn = config.connectDB();
        ...
        if (rs.next()) {
            ...
            // Load profile picture (BLOB or TEXT path)
            ...
            // Always set the profile icons, using the icon (which may be default)
            if (icon != null) {
                ImageIcon resized = resizeImage(icon, 150, 120);
                forEDITPICTURE.setIcon(resized);
                savePicHereFirst.setIcon(resized);

                int profileWidth = Math.max(cicrcle_profile.getWidth(), 50);
                int profileHeight = Math.max(cicrcle_profile.getHeight(), 60);
                if (profileWidth <= 0) profileWidth = 60;
                if (profileHeight <= 0) profileHeight = 60;
                int diameter = Math.min(profileWidth, profileHeight);
                cicrcle_profile.setIcon(createCircularImageIcon(resized, diameter, diameter));
                cicrcle_profile.setText("");
                forEDITPICTURE.revalidate();
                forEDITPICTURE.repaint();
                savePicHereFirst.revalidate();
                savePicHereFirst.repaint();
                cicrcle_profile.revalidate();
                cicrcle_profile.repaint();
            } else {
                // Fallback if no icon at all
                cicrcle_profile.setText("No Image");
                cicrcle_profile.revalidate();
                cicrcle_profile.repaint();
            }
        }
...
try (PreparedStatement ps2 = conn.prepareStatement(
     "SELECT specialty FROM tbl_dentists WHERE acc_id=?"
)) {
    ps2.setInt(1, session.getId());
    ResultSet rs2 = ps2.executeQuery();
    if (rs2.next()) {
        String specialty = rs2.getString("specialty");
        if (specialty != null && !specialty.trim().isEmpty()) {
            set_specialization.setSelectedItem(specialty); // combo box
            displaySpecialty.setText(specialty);           // label
        }
    }
}
...
private void saveScheduleToDB() {
    String start = startTimeCombo.getSelectedItem().toString();
    String end   = endTimeCombo.getSelectedItem().toString();

    // Collect selected days into a comma-separated string
    List<String> days = new ArrayList<>();
    if (monCheck.isSelected()) days.add("Monday");
    if (tueCheck.isSelected()) days.add("Tuesday");
    if (wedCheck.isSelected()) days.add("Wednesday");
    ...
    try (Connection conn = config.connectDB();   // ✅ use connectDB() not getConnection()
         PreparedStatement ps = conn.prepareStatement(
             "UPDATE tbl_dentists SET work_start = ?, work_end = ?, work_days = ? WHERE dentist_id = ?"
         )) {
        ...
        ps.setInt(4, session.getId()); // dentist_id from session
        ps.executeUpdate();
...
private void loadTodaySchedule() {
    ...
    try (Connection conn = config.connectDB()) {
        // First, show the next confirmed appointment if any.
        try (PreparedStatement pst = conn.prepareStatement(
                "SELECT a.app_date, a.app_time, a.app_service, COALESCE(NULLIF(TRIM(p.pat_name), ''), 'Patient') AS pat_name " +
                "FROM tbl_appointments a " +
                "LEFT JOIN tbl_patients p ON p.pat_id = a.pat_id " +
                "WHERE (a.dentist_id = ? OR a.dentist_id = ?) " +
                "AND LOWER(a.app_status) = 'confirmed' " +
                "AND DATE(a.app_date) >= DATE('now') " +
                "ORDER BY DATE(a.app_date) ASC, TIME(a.app_time) ASC " +
                "LIMIT 1"
        )) {
            pst.setInt(1, dentistId);
            pst.setInt(2, session.getId());
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
                    ...
                    String patientName = rs.getString("pat_name");
                    upcoming.setText("Upcoming: " + patientName + " at " + time);
                    return;
                }
            }
        }

        // No confirmed appointment found, show today's dentist schedule if available.
        try (PreparedStatement ps = conn.prepareStatement(
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    String days = rs.getString("work_days");
                    DayOfWeek day = today.getDayOfWeek();
                    if (days != null && days.toUpperCase().contains(day.toString())) {
                        upcoming.setText("Today: " + rs.getString("work_start") + " - " + rs.getString("work_end"));
                    } else {
                        upcoming.setText("No schedule today");
                    }
                } else {
                    upcoming.setText("No schedule today");
                }
        ...
        e.printStackTrace();
        JOptionPane.showMessageDialog(this, "Error loading today's schedule: " + e.getMessage());
    }
}
...
private void loadScheduleTable() {
    try (Connection con = config.connectDB();
        ...
        pst.setInt(1, session.getDentistId()); // ✅ correct ID
        ResultSet rs = pst.executeQuery();

        // Define table model (non-editable)
        javax.swing.table.DefaultTableModel model = new javax.swing.table.DefaultTableModel(
            new Object[]{"Start Time", "End Time", "Work Days"}, 0
        ) {
            @Override
            public boolean isCellEditable(int row, int column) {
        ...
        while (rs.next()) {
            ...
            model.addRow(new Object[]{start, end, days});
        }

        scheduleTable.setModel(model); // ✅ bind model to JTable
...
// Header styling
javax.swing.table.JTableHeader header = scheduleTable.getTableHeader();
header.setFont(new Font("Segoe UI", Font.BOLD, 14));
header.setBackground(new Color(200, 230, 240)); // soft healthcare blue
header.setForeground(Color.DARK_GRAY);

// Column widths
scheduleTable.getColumnModel().getColumn(0).setPreferredWidth(90);   // Start Time
scheduleTable.getColumnModel().getColumn(1).setPreferredWidth(90);   // End Time
scheduleTable.getColumnModel().getColumn(2).setPreferredWidth(250);  // Work Days

// Center align Start/End times
...
// Work Days stays left-aligned for readability
// ✅ Alternating row colors (clinic feel)
scheduleTable.setDefaultRenderer(Object.class, new javax.swing.table.DefaultTableCellRenderer() {
    ...
    public java.awt.Component getTableCellRendererComponent(
        ...
        if (!isSelected) {
            if (row % 2 == 0) {
                c.setBackground(new Color(245, 250, 255)); // very light dental blue
            } else {
        ...
        JOptionPane.showMessageDialog(this, "Error loading schedule table: " + e.getMessage());
    }
}
  /**
    ...
    private javax.swing.JTable Table_PatientsofDentist;
    private javax.swing.JButton btnConfirm;
    private javax.swing.JButton btnReject;
    private javax.swing.JButton btnComplete;
    private javax.swing.JButton btnSetStartTime;
